        ...
        <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
          <Col xs={24} sm={8}>
            <Card className="glass-card">
              <Statistic
                title="头酒 (5%)"
                value={simulation.liquorStorage.headLiquor.toFixed(1)}
                suffix="kg"
                valueStyle={{ color: '#ff7a45' }}
                prefix={<Tag color="orange" style={{ marginRight: 0 }}>酒头</Tag>}
              />
            </Card>
          </Col>
          <Col xs={24} sm={8}>
            <Card className="glass-card">
              <Statistic
                title={`中段优级 (85%) · ${simulation.liquorStorage.midAlcoholDegree.toFixed(1)}%vol`}
                value={simulation.liquorStorage.midLiquor.toFixed(1)}
                suffix="kg"
                valueStyle={{ color: '#52c41a' }}
                prefix={<Tag color="green" style={{ marginRight: 0 }}>中流</Tag>}
              />
            </Card>
          </Col>
          <Col xs={24} sm={8}>
            <Card className="glass-card">
              <Statistic
                title="尾酒 (10%)"
                value={simulation.liquorStorage.tailLiquor.toFixed(1)}
                suffix="kg"
                valueStyle={{ color: '#13c2c2' }}
                prefix={<Tag color="cyan" style={{ marginRight: 0 }}>酒尾</Tag>}
              />
            </Card>
          </Col>
        </Row>
...
/** 智能格式化数值: 数字 → 整数(>=100) / 1位小数(>=1) / 2位小数(<1)；非数字原样返回 */
function fmtNumber(v: any): any {
