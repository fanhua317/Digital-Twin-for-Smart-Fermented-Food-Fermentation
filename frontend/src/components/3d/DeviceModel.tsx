...
function DeviceModelImpl({ position, deviceNo: _deviceNo, name, type, status, onClick, materialInfo }: DeviceModelProps) {
...
export default memo(DeviceModelImpl, (prev, next) =>
  prev.deviceNo === next.deviceNo &&
  prev.name === next.name &&
  prev.type === next.type &&
  prev.status === next.status &&
  // 物料数值按 5kg 颗粒度比较，避免每次小幅波动都触发整树重建
  Math.abs((prev.materialInfo?.inputLevel ?? 0) - (next.materialInfo?.inputLevel ?? 0)) < 5 &&
  Math.abs((prev.materialInfo?.outputLevel ?? 0) - (next.materialInfo?.outputLevel ?? 0)) < 5 &&
